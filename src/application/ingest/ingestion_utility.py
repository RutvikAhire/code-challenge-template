...
class IngestionUtility:
    ...
    def __init__(self, ingestion_files, ingestion_type):
        ...
        self.log_file_path = f'{self.log_folder_path}/ingestion_{self.setup_datetime_str}.log'
        ...
        self.logger.info(
            'Number of wx_data files to be processed: %s', f'{len(self.ingestion_files)}')
        self.logger.info('Ingestion request endpoint: %s',
                         f'{self.ingestion_type}')
    ...
    def ingestor(self):
        try:
            ...
            for file in self.ingestion_files:
                file_station_id = str(file.split(
                    f'{self.wx_data_directory}/')[0].split('.txt')[0])
                self.logger.info('Processing wx_data file: %s',
                                 f'{file_station_id}.txt')

                # Read file data into 'file_df' dataframe
                file_df = pd.read_csv(
                    filepath_or_buffer=f'{self.wx_data_directory}/{file_station_id}.txt', sep='\t', header=None)
                row_counter = 0
                while row_counter < len(file_df):
                    ...
                    result = (
                        record_reading_id in distinct_reading_id_dict)
                    if result == 0:
                        ...
                        record_max_temperature = int(str(row_data[1])) if str(
                            row_data[1]) != '-9999' else None
                        record_min_temperature = int(str(row_data[2])) if str(
                            row_data[2]) != '-9999' else None
                        record_precipitation = int(str(row_data[3])) if str(
                            row_data[3]) != '-9999' else None
                        record_dict = {
                            ...
                            'month': record_month,
                            'day': record_day,
                            'max_temperature': round(record_max_temperature*0.10, 4) if record_max_temperature is not None else record_max_temperature,
                            'min_temperature': round(record_min_temperature*0.10, 4) if record_min_temperature is not None else record_min_temperature,
                            'precipitation': round(record_precipitation*0.01, 4) if record_precipitation is not None else record_precipitation
                        }
                        bulk_insert_list.append(record_dict)
                    row_counter = row_counter + 1
            self.logger.info(
                'Number of bulk insert records identified: %s', f'{len(bulk_insert_list)}')
            if len(bulk_insert_list) > 0:
                self.logger.info(
                    'START: Bulk records insertion')
