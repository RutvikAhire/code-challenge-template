...
class ApiWeather(Resource):
    ...
    @api.response(200, description='OK', message='OK')
    @api.response(404, description='Error: Not Found', message='Error: Not Found')
    def get(self):
        """
        ...
        parser.add_argument(
            'page', required=False, type=str, help='Required format: 1')
        ...
        if date is not None and station is not None:
            if len(date) == 8 and len(station) == 11:
                ...
                try:
                    ...
                    return {"endpoint": "/weather", "output_count": readings.total, "current_page": paginate_response_op[1], "total_pages": readings.pages, "args": {"date": date, "station": station, "page": page}, "response": serialized_output}
                except NoResultFound:
                    return {"message": "Records for date and station could not be found."}, 404
            ...
            if len(station) == 11:
                ...
                try:
                    ...
                    return {"message": "Records for station could not be found."}, 404
            ...
            if len(date) == 8:
                ...
                try:
                    ...
                    return {"message": "Records for date could not be found."}, 404
            else:
                return {"message": "Incorrect date format."}, 404
        else:
            readings = Readings.query.paginate(per_page=1000)
            paginate_response_op = paginate_response(
                paginate_object=readings, args=args)
            # Serialize the queryset
            serialized_output = readings_schema.dump(paginate_response_op[0])
            return {"endpoint": "/weather", "output_count": readings.total, "current_page": paginate_response_op[1], "total_pages": readings.pages, "args": {"date": date, "station": station, "page": page}, "response": serialized_output}
        ...
        if year is not None and station is not None:
            if len(year) == 4 and len(station) == 11:
                ...
                try:
                    ...
                    serialized_output = results_schema.dump(
                        paginate_response_op[0])
                    # return {"endpoint": "/weather/stats", "current_page": page, "total_pages": results.pages, "args": {"year": year, "station": station}, "response": serialized_output}
                    return {"endpoint": "/weather/stats", "output_count": results.total, "current_page": paginate_response_op[1], "total_pages": results.pages, "args": {"year": year, "station": station, "page": page}, "response": serialized_output}
                except NoResultFound:
                    return {"message": "Stats for year and station could not be found."}, 400
            ...
            if len(station) == 11:
                ...
                try:
                    ...
                    return {"message": "Stats for station could not be found."}, 400
            ...
            if len(year) == 4:
                ...
                try:
                    ...
                    return {"message": "Stats for year could not be found."}, 400
        else:
            results = Results.query.paginate(per_page=1000)
            paginate_response_op = paginate_response(
                paginate_object=results, args=args)
            # Serialize the queryset
            serialized_output = results_schema.dump(paginate_response_op[0])
            # return {"endpoint": "/weather/stats", "current_page": page, "total_pages": results.pages, "args": {"year": year, "station": station}, "response": serialized_output}
            return {"endpoint": "/weather/stats", "output_count": results.total, "current_page": paginate_response_op[1], "total_pages": results.pages, "args": {"year": year, "station": station, "page": page}, "response": serialized_output}
