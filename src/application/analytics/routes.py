...
def analytics_hub():
    ...
    analytics_hub.report_files = os.listdir(analytics_hub.results_directory)
    ...
    if len(analytics_hub.report_files) == 0:
        ...
        analytics_hub.latest_report_filepath = (
            f"{analytics_hub.results_directory}/{analytics_hub.report_filename}"
        )
        with open(analytics_hub.latest_report_filepath, mode="r") as infile:
            analytics_hub.result_json = json.load(infile)
    return render_template(
        "analytics/analytics_hub.html",
        title="Analytics",
        report_json=analytics_hub.result_json,
    )
...
def analytics_download_report():
    ...
    analytics_download_report.results_directory = (
        f"{analytics_download_report.root_directory}/results"
    )
    ...
    if len(analytics_hub.report_files) == 0:
        ...
        analytics_download_report.report_filename = (
            f"{analytics_download_report.report_files[-1]}"
        )
    ...
    return send_file(
        analytics_download_report.latest_report_filepath, as_attachment=True
    )
